...
if ($#ARGV != 3 || $ARGV[0] eq "-h") 
	...
	print "Ambiguous mapping is evaluated at the level of components: if a read matches two sequences\n";
	print "from the same component, that read is counted. If a read matches two sequences from different\n";
	print "components, that read is discarded.\n\n";
...
# -- loop through all matches for each read and evaluate whether more than one component was hit
...
# -- exclude reads mapping to multiple components
	$niga = @iga;
	if ($niga > 1) {$ambig++; next;}

# -- count passing reads by component
	$unambig++;
	$ch{$iga[0]}++;
	if (@ama==1) {$singref++;}
	}
...
print $multcount, " reads mapped to multiple reference sequences equally well.\n";
print $ambig, " excluded for matching different components equally well.\n";
print $singref, " showed unambiguous matches to a single reference sequence.\n";
print $unambig, " showed unambiguous matches to a single component.\n";
